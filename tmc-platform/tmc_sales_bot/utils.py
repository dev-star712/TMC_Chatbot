...
from config import ModelType, VEHICLE_WITH_VIDEO_JSON, VALIDATE_POSTCODE, CALC_DISTANCE, ModelChoice
...
def get_vehicle_data(info):
    with open(join(VEHICLE_WITH_VIDEO_JSON, info["Name"]+".json"), "r") as f:
        vehicle_detail = json.load(f)
    vehicle_data = vehicle_detail["data"]["results"]
    return vehicle_data


def get_company_vehicle_reference(vin: str, info):    
    vehicle_data = get_vehicle_data(info)
    vin = vin.split("/")
    if vin[-1] == "":
        vin = vin[-2]
    else:
    ...
    for vehicle in vehicle_data:
        if vin.lower() == vehicle['vehicle']['vin'].lower():
            ...
            # reference["website_link"] = info["URLS"]["FRONTEND_URL"] + "/vehicles-for-sale/viewdetail/" + vehicle['vehicle']['vin']
            reference["website_link"] = info["URLS"]["FRONTEND_URL"] + "/autotradervehiclelocator?VRM=" + vehicle['vehicle']['registration']
...
def get_vin_from_vrn(vrn: str, info):
    vehicle_data = get_vehicle_data(info)
    for vehicle in vehicle_data:
        # print(vin.lower(), vehicle['vehicle']['vin'].lower())
        if vrn.lower() == vehicle['vehicle']['registration'].lower():
            return vehicle['vehicle']["vin"]
    if validate_vrn(vrn, info):
        return "vehicle not found"
    else:
        return "invalid vrn"


def get_vin_from_url(url: str, info):
    hostname = urlparse(url).hostname
    frontend_host = urlparse(info["URL"]["FRONTEND_URL"]).hostname
...
def check_company_vehicle(vin: str, info):
    vehicle_data = get_vehicle_data(info)
...
def check_company_vehicle_vrn(vrn: str, info):
    vehicle_data = get_vehicle_data(info)
...
def get_video_link(vin: str, info):
    vehicle_data = get_vehicle_data(info)
...
def get_forecourt_price(vin: str, info):
    vehicle_data = get_vehicle_data(info)
...
def validate_vrn(vrn: str, info):
    ...
    response = requests.request("POST", info["URLS"]["VALUATION_URL"], headers=headers, data=payload)
...
def valuate_vehicle(vrn: str, mileage: str, condition: str, service_history: str, name: str, email: str, number: str, info):
    payload = json.dumps({
        "registration": vrn,
        "miles": mileage,
        "condition" : condition,
    ...
    if response.status_code == 200:
        ...
        # with open("./../valuation_template.txt", "w") as f:
...
def get_vehicle_description(vehicle, info):
    ...
    for prop in important_props:
        if prop in vehicle["vehicle"].keys() and vehicle["vehicle"][prop] != None:
            ...
            description += str(prop) + ' is ' + info["URLS"]["FRONTEND_URL"] + "/vehicles-for-sale/viewdetail/" + vehicle['vehicle']['vin'] + ', and '
...
def calculate_distance(postcode: str, vin: str):
    ...
    response = response.json()
    print ("###CalcResponse", response)
    return response


def finance_limit(vin: str, info):
    if validata_vin(vin, info):
        url = info["URLS"]["FINANCE_LIMIT_URL"] + "?vin={vin_number}"
        url = url.format(vin_number=vin)
        payload = {}
        headers = {}
        response = requests.request("GET", url, headers=headers, data=payload)
        response = response.json()
        # with open('./../finance_limit_template.txt', 'w') as f:
        #     json.dump(response, f, indent=4)
        return response
    else:
        return {
...
def finance_calc(vin: str, term: int, deposit: int, info):
    vin = vin.upper()
    if validata_vin(vin, info):
        url = info["URLS"]["FINANCE_CALC_URL"] + "?vin={vin}&term={term}&deposit={deposit}"
        url = url.format(vin=vin, term=term, deposit=deposit)
        payload = {}
        headers = {}
        print ("LLLLLLLLLLLLLL", url)
        response = requests.request("GET", url, headers=headers, data=payload)
        response = response.json()
        # with open('./../finance_calc_template.txt', 'w') as f:
        #     json.dump(response, f, indent=4)
        return response
    else:
        return {
...
def send_sms(number: str, message: str, info):
    headers = {
        'Content-Type': 'application/json'
    }
    body = json.dumps({"To": number, "Body": message, "type": "sms"})
    response = requests.request("POST", info["URLS"]["SEND_SMS_URL"], headers=headers, data=body)
    print("PROP", response.status_code, response.text, "SMS")
    # with open('./../send_sms_template.txt', 'w') as f:
    #         json.dump(response.json(), f, indent=4)
    return response.status_code


def send_whatsapp(number: str, message: str, info):
    headers = {
        'Content-Type': 'application/json'
    }
...
def send_email(email: str, message: str, title: str, info):
    headers = {
        'Content-Type': 'application/json'
    }
    body = json.dumps({"To": email, "Body": message, "title": title})
    response = requests.request("POST", info["URLS"]["SEND_EMAIL_URL"], headers=headers, data=body)
    # with open('./../send_email_template.txt', 'w') as f:
    #     json.dump(response.json(), f, indent=4)
    return response.status_code

def retrieve_vehicle(registration: str, info):
    headers = {
        'Content-Type': 'application/json'
    }
    body = json.dumps({"registration": registration})
    response = requests.request("POST", info["URLS"]["RETRIEVE_VEHICLE_URL"], headers=headers, data=body)
    if response.status_code != 200 :
        return ""
    
    data = json.loads(response.text)
    # with open('./../retrieve_vehicle_template.txt', 'w') as f:
...
def retrieve_vehicle_info(registration: str, info):
    headers = {
        'Content-Type': 'application/json'
    }
    body = json.dumps({"registration": registration})
