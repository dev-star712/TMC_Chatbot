...
import re
from config import VEHICLE_WITH_VIDEO_JSON, SQL_DB
...
def create_database(vehicle_data, info):
    conn = sqlite3.connect(join(SQL_DB, info["Name"]+".db"))
    cursor = conn.cursor()
...
Only generate query.
Pickup is for 'bodyType' field, not 'vehicleType'.
...
def get_vehicle_data(vehicle_data, customer_conversation, features, info):
    ...
    banned =['manualtransmission']
    for feature in features :
        if feature.lower() not in sql_query.lower() and feature.lower().replace(" ", "") not in banned:
    ...
    all_vehicles = read_vehicle_data(info)
            ...
            if isinstance(prop, str):
                if len(prop) == 17 and " " not in prop:
                    ...
                    if set(rfeatures).issubset(set(cfeatures)) :
                        print (cfeatures)
                        final.append(info["URLS"]["FRONTEND_URL"] + r"/vehicles-for-sale/viewdetail/{}".format(prop))
                        break
    return final

def getVehicleType(vrn, info) :
    type = retrieve_vehicle(vrn, info)
    print ("*******", type)
    return type
