...
from tmc_sales_bot.utils import embedding, valuate_vehicle, get_company_vehicle_reference, \
    send_whatsapp, send_sms, send_email, calculate_distance, validate_postcode, finance_calc, check_company_vehicle_vrn, get_completion, retrieve_vehicle_info
...
class ChatBot:
    def __init__(self):
        ...
        self.state_variables = {"user_contact_info": {"name": "", "e-mail": "", "number": ""},
                                ...
                                "finance_info": {"vin": "", "deposit": 0, "term": 0, "active": ""},
                                "full_pay_info": {"vin": "", "active": ""},
                                "user_location": {}, "viewed_vehicles": [], "stripe": ""}
        self.company_info = {}
        self.chat_mode = "guest"
        self.index = pc.Index(environ['PINECONE_INDEX_NAME'])
        

    def set_company_info(self, token):
        try :
            with open('./fixture/accounts/accounts.json', "r") as f:
                accounts = json.load(f)
                myAccount = list(filter(lambda x: x["AccessKey"] == token, accounts))[0]
                self.company_info = myAccount
                with open(join(VEHICLE_WITH_VIDEO_JSON, self.company_info["Name"]+".json"), "r") as f:
    ...
    def query_db(self, query, property=None, top_k=5):
        print ("Calling pinecone", ("Chatbot-"+self.company_info["Name"]).replace(" ", "").lower(), property)
        query_vector = embedding(query)
        if property is not None:
            result = self.index.query(
                vector=query_vector, filter={"prop":property}, top_k=top_k, namespace=("Chatbot-"+self.company_info["Name"]).replace(" ", "").lower())
        else:
            result = self.index.query(
                vector=query_vector, top_k=top_k, namespace=("Chatbot-"+self.company_info["Name"]).replace(" ", "").lower())
        ...
        if ids == []:
            ...
            data = self.index.fetch(ids, namespace=("Chatbot-"+self.company_info["Name"]).replace(" ", "").lower()).to_dict()["vectors"]
            descriptions = []
            for id in ids:
    ...
    def compose_prompt(self, query):
        ...
        if "user_vehicle_registration_number" in params.keys() and params["user_vehicle_registration_number"] is not None:
            if check_company_vehicle_vrn(params["user_vehicle_registration_number"], self.company_info):
        ...
        if "conversation_intent" in params.keys() and params["conversation_intent"] is not None:
            ...
            if intent == "searching_vehicle":
                recommended_vehicles = get_vehicle_data(self.vehicle_data, self.format_messages(), params["features"], self.company_info)
                ...
                try :
                    ...
                    if self.state_variables["user_vehicle_info"]["cost"] == "Missing Parameters." :
                        print ("LLLLLLLLLL")
                        reference_data = 'Missing parameters.'  
                    elif not isinstance(self.state_variables["user_vehicle_info"]["cost"], int) :
                        print ("^^^^%%%%")
                        ...
                        reference_data = "I can't valuate your vehicle. We will have a valuation expert contact you."
                except Exception as e:
                    print (e)
                    pass
            elif intent == "total_interest_payable":
                try :
                    print ("KKKKKK", self.state_variables["finance_info"], self.state_variables["viewed_vehicles"])
                    if self.state_variables["finance_info"]["deposit"] >= 0 and self.state_variables["finance_info"]["term"] >= 0 and len(self.state_variables["viewed_vehicles"]) > 0:
                        vin = self.state_variables["viewed_vehicles"][-1]
                        print ("@@@", self.state_variables["finance_info"])
                        payment_calc = finance_calc(vin, self.state_variables["finance_info"]["term"], self.state_variables["finance_info"]["deposit"], self.company_info)
                        print (payment_calc)
                        ...
                        focused_vehicle_reference = get_company_vehicle_reference(focused_vehicle, self.company_info)
                        ...
                        reference_data = f"The total interest payable is {payable}"
                    else :
                        reference_data = "You must provide deposit and term details."
                except Exception as e:
                    print (e)
                    reference_data = "Deposit or term information is incorrect."
        ...
        if "user_post_code" in params.keys() :
            calDis = calculate_distance(params["user_post_code"], '')
            if calDis["message"] == "success":
                distance = calDis["result"]["distance"]
                print ("###Included", distance)
                reference_data += f' The distance between your location and company location is {distance} miles. '
    ...
    def compose_user_finance_info(self, parameters: dict) -> str:
        ...
        payment_calc = finance_calc(vin, term, deposit, self.company_info)
        print (";;;;;; finance: ", vin, term, deposit, payment_calc)
        regular_payment = ""
        apr=""
        ...
        if payment_calc["message"] == "Success!":        
            ...
            if payment_calc["data"]["vatStatus"]:
                deposit = r"Original Deposit value is {}. This vehicle include VAT so user need to pay extra {}.".format(payment_calc["data"]["Deposit"], payment_calc["data"]["vatPrice"])
    ...
    def compose_user_vehicle_info(self, parameters: dict) -> str:
        for key in parameters.keys():
            if key == "user_vehicle_registration_number" and parameters["user_vehicle_registration_number"] != "" and parameters["user_vehicle_registration_number"] is not None:
                ...
                self.state_variables["user_vehicle_info"]["type"] = getVehicleType(parameters["user_vehicle_registration_number"], self.company_info)
        ...
        if self.state_variables["user_vehicle_info"]["vrn"] != "" and self.state_variables["user_vehicle_info"][
            "mileage"] != "":
            if check_company_vehicle_vrn(self.state_variables["user_vehicle_info"]["vrn"], self.company_info):
                ...
                print ("#Valuate", self.state_variables["user_vehicle_info"])
                if "condition" not in self.state_variables["user_vehicle_info"].keys() :
                    self.state_variables["user_vehicle_info"]["condition"] = ""
                valuation_res = valuate_vehicle(
                    ...
                    self.state_variables["user_contact_info"]["name"], self.state_variables["user_contact_info"]["e-mail"], self.state_variables["user_contact_info"]["number"], self.company_info
                )
        ...
        if self.state_variables["user_vehicle_info"]["vrn"] != "" :
            res = retrieve_vehicle_info(self.state_variables["user_vehicle_info"]["vrn"], self.company_info)
            print ("`````````````````````"           , res)
            if res != "" :
                self.state_variables["user_vehicle_info"]["make"] = res['vehicle']['make']
                self.state_variables["user_vehicle_info"]["model"] = res['vehicle']['model']
                self.state_variables["user_vehicle_info"]["generation"] = res['vehicle']['generation']
                
        print ("&&&&&&&&&", self.state_variables["user_vehicle_info"])
    ...
    def update_message_history(self, history):
        message_history = []
        message_history.append({"role": "system", "content": get_prompt_template(PromptTemplate.SYSTEM_PROMPT).replace("{Company_Name}", self.company_info["Name"])})
    ...
    def run_function(self, func):
        ...
        if function_name == "send_whatsapp":
            ...
            if number == "0" and self.state_variables["user_contact_info"]["number"] == "":
                ...
                send_whatsapp(number, message, self.company_info)
            ...
            if number == "0" and self.state_variables["user_contact_info"]["number"] == "":
                ...
                if number == "0":
                    number = self.state_variables["user_contact_info"]["number"]
                number = "+" + number
                send_sms(number, message, self.company_info)
            ...
            if email == "" and self.state_variables["user_contact_info"]["e-mail"] == "":
                ...
                if email == "":
                    email = self.state_variables["user_contact_info"]["e-mail"]
                send_email(email, message, title, self.company_info)
