...
export default function SPSBtns({ vin, openShareModel }) {
  const navigate = useNavigate();
  const shortlist = parseLocalStorageValue("shortlist") || [];
  return (
    <div className="px-5 flex flex-row">
      {!shortlist.includes(vin) && (
        <div
          onClick={() => {
            if (!shortlist.includes(vin)) {
              shortlist.push(vin);
              localStorage.setItem("shortlist", JSON.stringify(shortlist));
              navigate("/shortlist");
            }
          }}
          className="flex   cursor-pointer justify-center items-center text-sm leading-5 tracking-wider font-semibold font-open-sans-condensed sm:font-open-sans border-2 border-bray-800 px-4 py-2 rounded-full hover:bg-gray-400  bg-black text-white"
        >
      ...
      {/* <div className="flex  cursor-pointer ml-4 justify-center items-center  text-sm leading-5 tracking-wider font-semibold font-open-sans-condensed sm:font-open-sans border-2 border-black px-4 py-2 rounded-full hover:bg-gray-400  bg-white text-black">
      ...
      <div
        onClick={() => openShareModel(true)}
        className=" cursor-pointer flex ml-4 justify-center items-center text-sm leading-5 tracking-wider font-semibold font-open-sans-condensed sm:font-open-sans border-2 border-black px-4 py-2 rounded-full hover:bg-gray-400  bg-white text-black"
      >
