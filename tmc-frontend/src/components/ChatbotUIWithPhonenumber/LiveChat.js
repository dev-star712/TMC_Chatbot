...
export default function LiveChat() {
  ...
  const page = useSelector((state) => state.chatbot.page);

  const setPage = (value) => {
    dispatch(movePage(value));
  };

  const guest = useSelector((state) => state.chatbot.guest);
  ...
  const load = useSelector((state) => state.chatbot.load);

  const getHistory = async (verified) => {
    ...
    if (!load || verified) {
      try {
        ...
        const response = await axios.get(
          `https://${process.env.REACT_APP_API}/api/bot/history?limit=30`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const checkSMS = async (phone_number) => {
    try {
      ...
      if (initialRemainingTime <= 0) {
        ...
        const response = await axios.get(
          `https://${process.env.REACT_APP_API}/api/twilio/checksms`,
          {
            params: {
              phone_number,
            },
  ...
  const verifySMS = async (phone_number, verification_code) => {
    try {
      const response = await axios.post(
        `https://${process.env.REACT_APP_API}/api/bot/verify`,
        {
          phone_number,
          verification_code,
        }
  ...
  useEffect(() => {
    if (isBotChatOpen) {
      ...
      if (jwtToken) {
        ...
        // console.log(parseLocalStorageValue("extraAssistant"));

        dispatch(initMessage(parseLocalStorageValue("messages") || []));
        // else localStorage.removeItem("extraAssistant");
      }
    }
  ...
  useEffect(() => {
    ...
    const last_page = parseLocalStorageValue("page") || 0;
    setPage(last_page);
    if (flag) {
      openBotChat();
    }
