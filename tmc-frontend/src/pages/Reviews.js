...
export default function Reviews() {
  ...
  const getReviewsData = async () => {
    setLoading(true);
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/allreview`;
    await axios
      .get(url)
      .then((response) => {
  ...
  const getData = async () => {
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/load?page=0-12-0`;
    await axios
      .get(url)
      .then((response) => {
        if (response.status === 200) {
    ...
    <MainLayout>
      {sdata && (
        <Meta
          ...
          canonical_url="/motoring-hub/reviews/"
        />
      )}
      <HeroSection1 img={null} words={["motoring hub review"]} mt={"mt-0"} />
