...
export default function FCADisclaimer() {
  const [data, setData] = useState(null);

  const getData = async () => {
    setLoading(true);
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/load?page=0-4-0`;
    await axios
      .get(url)
      .then((response) => {
        setLoading(false);
    ...
    <MainLayout>
      {data && (
        <Meta
          ...
          canonical_url="/fca-disclaimer/"
        />
      )}
      <Typography variant="h1" className="text-center py-0 sm:py-4">
        {data ? data.title || "TMC FCA Disclaimer" : "TMC FCA Disclaimer"}
      </Typography>
      {data && (
