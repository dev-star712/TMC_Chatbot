...
import usedcarimg from "../assets/images/usedcars.png";
...
export default function UsedTrucks() {
  ...
  const getStockData = async () => {
    setLoading(true);
    const url = `https://${process.env.REACT_APP_API}/api/vehicle/stock?page=1&pageSize=ALL&vehicleType=Car`;
    await axios
      .get(url)
      .then((response) => {
  ...
  const getData = async () => {
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/load?page=0-7-0`;
    await axios
      .get(url)
      .then((response) => {
        if (response.status === 200) {
    ...
    <MainLayout>
      {data && (
        <Meta
          ...
          canonical_url="/vehicles-for-sale/used-cars/"
        />
      )}
