...
export default function Article() {
  ...
  const getNewsData = async () => {
    setLoading(true);
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/allnews`;
    await axios
      .get(url)
      .then((response) => {
  ...
  const getData = async () => {
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/load?page=0-14-0`;
    await axios
      .get(url)
      .then((response) => {
        if (response.status === 200) {
    ...
    <MainLayout>
      {sdata && (
        <Meta
          ...
          canonical_url="/motoring-hub/blog/"
        />
      )}
