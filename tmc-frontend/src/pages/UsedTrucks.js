...
export default function UsedTrucks() {
  ...
  const getData = async () => {
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/load?page=0-6-0`;
    await axios
      .get(url)
      .then((response) => {
        if (response.status === 200) {
    ...
    <MainLayout>
      {data && (
        <Meta
          ...
          canonical_url="/vehicles-for-sale/used-trucks/"
        />
      )}
        ...
        <div className="flex flex-col md:flex-row md:justify-between max-w-[1440px] ">
          ...
          <div className="w-full lg:w-[72%]  flex-col">
            ...
            <div className="px-5 w-full ">
              {showlist.length !== 0 ? (
                <div className="flex w-full flex-wrap items-center justify-center gap-1 xl:gap-x-16 2xl:gap-1">
                  {viewlist.map((ele) => {
                    let price = "None";
