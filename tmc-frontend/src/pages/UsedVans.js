...
import usedvanimg from "../assets/images/usedvans.png";
...
export default function UsedTrucks() {
  ...
  const getStockData = async () => {
    setLoading(true);
    const url = `https://${process.env.REACT_APP_API}/api/vehicle/stock?page=1&pageSize=ALL&vehicleType=Van`;
    await axios
      .get(url)
      .then((response) => {
        if (response.status === 200) {
          ...
          setTaxonomy(extractTaxonomy(vehicles, true));
  ...
  const getData = async () => {
    const url = `https://${process.env.REACT_APP_CMS_API}/api/content/load?page=0-8-0`;
    await axios
      .get(url)
      .then((response) => {
        if (response.status === 200) {
    ...
    <MainLayout>
      {data && (
        <Meta
          ...
          canonical_url="/vehicles-for-sale/used-vans/"
        />
      )}
