...
const VideoDetailScreen = () => {
  ...
  const init = async () => {
    try {
      ...
      if (res.error) {
        ...
        setVideoUrl(res.data.article.video_url || "");
  ...
  const getCategories = async () => {
    try {
      const res = await getCategory({ type: "video" });
      console.log(res);
      if (res.error) {
        throw new Error(res.error.data.message);
      } else {
          ...
          <div className="m-2 bg-white">
            ...
            <Row className="mb-4" gutter={[24, 0]}>
              <Col span={3}>
                ...
                <strong style={{ color: "red" }}>*</strong>
                <p>
                  The date to show on the Video article. Video articles are
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  ...
                  navigate(`/content/1-2?delete=success`);
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                if (!video_url) {
                  throw new Error("Input the Video URL in Content Tab");
                } else if (!urlPattern.test(video_url)) {
                ...
                if (pageId === "video" && res.data.pageId) {
                  navigate(`/content/video/${res.data.pageId}?new=success`);
                  window.location.reload();
                }
                toast.success(res.data.message);
              } catch (error) {
